...
const data = [
    '3 month contract.',
     <a><a style={{fontWeight:'600'}}>$300 nett per month</a> charged on the first working day of the month via credit card payment.</a>,
  ...
  const data1 = [
    '  FREE 1 pair of Jancis Robinson stemless wine glass (worth $88).',
    '10% off tasting samples for our online ZOOM tasting events.'
  ];

Modal.setAppElement("#root");
const ViewStarter = (ids) => {
	...
	return (
		<>
            <button  onClick={openModal} type="button" className="btn btn-danger my-2 mx-3" style={{minWidth:"300px",borderRadius:"25px"}}>Starter Tier Subscription <i className="fas fa-arrow-right"></i></button>
            ...
            <i onClick={closeModal} className="fa fa-times" style={{float:'right'}} aria-hidden="true"></i>
                ...
                <h2 className="headLeft"  style={{fontFamily:"Jost",fontWeight:800,fontSize:"20px"}}>STARTER TIER SUBSCRIPTION</h2>
                <List
                 dataSource={data}
                 size="small"
                renderItem={item => (
                ...
                />
                <p>But WAIT, what if you are more committed? Sign up for a <a style={{fontWeight:'bold', padding:'0', margin:'0'}}>6 month</a> subs instead and get a further:</p>
                <List
                 dataSource={data1}
                 size="small"
                renderItem={item => (
